// File from https://github.com/mermaid-js/mermaid-live-editor
//
...
export const initEditor = (monacoEditor: Monaco): void => {
  ...
  const keywords: {
    ...
    classDiagram: {
      ...
      blockKeywords: ['class'],
      keywords: ['link', 'click', 'callback', 'call', 'href', 'cssClass', 'direction', 'TB', 'BT', 'RL', 'LR'],
    },
    stateDiagram: {
    ...
    gantt: {
      typeKeywords: ['gantt'],
      blockKeywords: [],
      keywords: ['title', 'dateFormat', 'axisFormat', 'todayMarker', 'section', 'excludes', 'inclusiveEndDates'],
    },
    requirementDiagram: {
  ...
  monacoEditor.languages.setMonarchTokensProvider('mermaid', {
    ...
    tokenizer: {
      ...
      journey: [
        ...
        [
          /(^\s*.+?)(:)(.*?)(:)(.*?)([,$])/,
          ['string', 'delimiter.bracket', 'number', 'delimiter.bracket', 'variable', 'delimiter.bracket'],
        ],
        [/,/, 'delimiter.bracket'],
